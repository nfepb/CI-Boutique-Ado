...
# If order form is not valid:
        ...
        if request.user.is_authenticated:
            try:
                # If auth, prefill fields with info
...
def checkout_success(request, order_number):
    ...
    # Associate order to user_profile upon checkout
    if request.user.is_authenticated:
        profile = UserProfile.objects.get(user=request.user)
        ...
        if save_info:
            # Add info to dictionnary to match user_profile model
            profile_data = {
            ...
            # Create an instance of user_profile form & update data
